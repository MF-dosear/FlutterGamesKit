...
- (void)checkNet{
    ...
    [FKSDK sdkCheckNetBlock:^(BOOL isNet) {
        if (isNet) {
            // 有网，重新加载
            if (self.isInit == false) {
                
                NSString *api = @"https://sqcdn.ayouhuyu.com/stoneage_feeling_release/index_ios_xmwiosjfzy_https.html?os=ios&pf=xmwiosjfzy&td_channelid=xmwiosjfzy&version=1.0.0";
            ...
            UIViewController *vc = [FKRouter currentVC];
...
- (void)alert{
    ...
    [[FKRouter currentVC] presentViewController:alert animated:true completion:nil];
}
...
- (void)psy:(NSDictionary *)params{
    ...
    [FKSDK sdkPsyWithParams:dict result:^(NSDictionary * _Nonnull result) {
...
-(void)submitRoleInfo:(NSDictionary *)params{
    ...
    [FKSDK sdkSubmitRoleWithParams:dict result:^(NSDictionary * _Nonnull result) {
...
- (void)shareWithMessage:(NSDictionary *)msg{
    ...
    [FKSDK sdkShareWithTitle:@"" image:@"" url:url result:^(NSDictionary * _Nonnull result) {
